    ...
    <v-card-text>
      <v-form class="mx-auto ml-20 mr-20">
        ...
        <v-text-field
          ...
          :rules="[() => !!repeated_password || 'This field is required', passwordsMatch]"
          :error-messages="errorMessages"
          hint="Password must contain minimum 8 characters, 1 uppercase, 1 lowercase and 1 number and 1 special character."
          prepend-icon="mdi-lock"
          @click:append="showPassword = !showPassword"
...
export default {
  ...
  computed: {
    ...
    passwordsMatch () {
          return () => (this.password === this.repeated_password) || `The passwords you entered don't match.`
     }
  },
  methods: {
    register() {
      this.$http
        .post("http://localhost:8090/application/auth/signup", {
          email: this.email,
          password: this.password,
          firstName: this.firstName,
          lastName: this.lastName,
        ...
        .catch((er) => {
          alert("Email is already taken! Please, try again!");
          this.email = "";
          this.password = "";
          this.firstName = "";
          this.lastName = "";
